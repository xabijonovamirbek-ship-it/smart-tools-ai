...
function ResumeTool() {
  ...
  const generate = async () => {
    ...
    try {
      const ll = { ru: "русском", uz: "o'zbek tilida", en: "English" };
      const raw = await callAI(`Создай резюме на ${ll[form.lang]} языке. Верни ТОЛЬКО JSON: {"name":"","jobTitle":"","contact":"","summary":"","experience":[{"role":"","company":"","desc":""}],"education":"","skills":[]} Данные: ${form.firstName} ${form.lastName}, ${form.phone}, ${form.email}, ${form.city}, ${form.jobTitle}, ${form.company}, ${form.experience}, ${form.education}, ${form.skills}`, "Return only valid JSON, no markdown.");
      const parsed = parseJSON(raw);
      setResult(parsed || { error: "Ошибка. Попробуй снова." });
    } catch { setResult({ error: "Ошибка. Попробуй снова." }); }
    setLoading(false);
  };
  const print = () => {
    const win = window.open("", "_blank");
    const ph = photo ? `<img src="${photo}" style="width:110px;height:110px;border-radius:50%;object-fit:cover;border:3px solid #7C3AED;margin-bottom:12px;"/>` : `<div style="width:110px;height:110px;border-radius:50%;background:#e0ddff;display:flex;align-items:center;justify-content:center;font-size:38px;margin-bottom:12px;">👤</div>`;
    win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Resume</title><style>*{margin:0;padding:0;box-sizing:border-box;}body{font-family:sans-serif;}.page{display:flex;min-height:100vh;}.sidebar{width:220px;background:#7C3AED;color:#fff;padding:32px 20px;}.sidebar h1{font-size:20px;font-weight:700;margin-bottom:4px;}.t{font-size:12px;opacity:.8;margin-bottom:20px;}.sec{font-size:10px;text-transform:uppercase;letter-spacing:1.5px;opacity:.7;margin:18px 0 8px;}.sidebar p{font-size:12px;line-height:1.6;opacity:.9;}.sk{display:inline-block;background:rgba(255,255,255,.2);border-radius:12px;padding:3px 10px;font-size:11px;margin:3px 2px;}.main{flex:1;padding:40px 36px;}.sh{font-size:13px;font-weight:700;color:#7C3AED;text-transform:uppercase;border-bottom:2px solid #7C3AED;padding-bottom:4px;margin:24px 0 12px;}.sum{font-size:13px;line-height:1.7;color:#444;}.ei{margin-bottom:14px;}.er{font-size:14px;font-weight:600;}.ec{font-size:12px;color:#7C3AED;margin-bottom:4px;}.ed{font-size:12px;color:#555;line-height:1.6;}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact;}}</style></head><body><div class="page"><div class="sidebar"><div style="display:flex;flex-direction:column;align-items:center;text-align:center">${ph}<h1>${result.name}</h1><div class="t">${result.jobTitle}</div></div><div class="sec">Контакты</div><p>${result.contact}</p><div class="sec">Навыки</div><div>${(result.skills||[]).map(s=>`<span class="sk">${s}</span>`).join("")}</div></div><div class="main"><div class="sh">О себе</div><p class="sum">${result.summary}</p><div class="sh">Опыт</div>${(result.experience||[]).map(e=>`<div class="ei"><div class="er">${e.role}</div><div class="ec">${e.company}</div><div class="ed">${e.desc}</div></div>`).join("")}<div class="sh">Образование</div><p class="sum">${result.education}</p></div></div></body></html>`);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <input style={S.input} placeholder="Компания" value={form.company} onChange={e => set("company", e.target.value)} />
      ...
      {result && !result.error && <div style={S.card}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
          {photo && <img src={photo} style={{ width: 56, height: 56, borderRadius: "50%", objectFit: "cover", border: "2px solid #7C3AED" }} />}
          <div><p style={{ color: "#fff", fontWeight: 700, fontSize: 18 }}>{result.name}</p><p style={{ color: "#a78bfa", fontSize: 13 }}>{result.jobTitle}</p><p style={{ color: "rgba(255,255,255,0.45)", fontSize: 12 }}>{result.contact}</p></div>
        </div>
        <p style={{ color: "rgba(255,255,255,0.75)", fontSize: 13, lineHeight: 1.7, marginBottom: 12 }}>{result.summary}</p>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 14 }}>{(result.skills || []).map((s, i) => <span key={i} style={{ background: "rgba(124,58,237,0.3)", color: "#c4b5fd", fontSize: 12, padding: "4px 12px", borderRadius: 999 }}>{s}</span>)}</div>
        <button onClick={print} style={{ width: "100%", padding: "12px 0", borderRadius: 14, background: "#fff", color: "#7C3AED", fontWeight: 700, fontSize: 14, border: "none", cursor: "pointer" }}>📥 Скачать PDF</button>
      </div>}
...
function PortfolioTool() {
  ...
  const generate = async () => {
    setLoading(true); setResult(null);
    try {
      const ln = form.lang === "uz" ? "o'zbek lotin" : form.lang === "en" ? "English" : "русском";
      const raw = await callAI(`Создай портфолио фрилансера на ${ln}. Верни ТОЛЬКО JSON: {"name":"","profession":"","tagline":"","bio":"","skills":[],"whyMe":[],"offer":""} Данные: ${form.name}, ${form.profession}, ${form.age}, ${form.city}, ${form.about}, ${form.skills}`, `Copywriter. Lang:${form.lang}. uz=Latin only. ru=Cyrillic only. JSON only.`);
      const parsed = parseJSON(raw);
      setResult(parsed || { error: "Ошибка. Попробуй снова." });
    } catch { setResult({ error: "Ошибка. Попробуй снова." }); }
    setLoading(false);
  ...
  const print = () => {
    const win = window.open("", "_blank");
    const ph = photo ? `<img src="${photo}" style="width:120px;height:120px;border-radius:50%;object-fit:cover;border:4px solid #7C3AED;margin-bottom:16px;"/>` : `<div style="width:120px;height:120px;border-radius:50%;background:linear-gradient(135deg,#7C3AED,#9333EA);display:flex;align-items:center;justify-content:center;font-size:48px;margin-bottom:16px;">👤</div>`;
    const contacts = [form.telegram ? `✈️ ${form.telegram}` : "", form.instagram ? `📸 ${form.instagram}` : ""].filter(Boolean).join(" | ");
    win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Portfolio</title><style>*{margin:0;padding:0;box-sizing:border-box;}body{font-family:sans-serif;background:#0f0c29;color:#fff;}.hero{background:linear-gradient(135deg,#1a1040,#2d1b69);padding:48px 32px;text-align:center;}h1{font-size:32px;font-weight:900;margin-bottom:8px;}.prof{font-size:18px;color:#a78bfa;margin-bottom:8px;}.body{padding:32px;}.sec{font-size:11px;text-transform:uppercase;letter-spacing:2px;color:#7C3AED;margin:24px 0 10px;}.bio{font-size:14px;line-height:1.8;color:rgba(255,255,255,0.8);}.skills{display:flex;flex-wrap:wrap;gap:8px;}.skill{background:rgba(124,58,237,0.2);border:1px solid rgba(124,58,237,0.4);color:#c4b5fd;padding:6px 14px;border-radius:999px;font-size:13px;}.why-item{background:rgba(255,255,255,0.05);border-left:3px solid #7C3AED;padding:12px 16px;border-radius:0 12px 12px 0;font-size:13px;margin-bottom:8px;}.offer{background:linear-gradient(135deg,rgba(124,58,237,0.2),rgba(147,51,234,0.2));border:1px solid rgba(124,58,237,0.3);border-radius:16px;padding:20px;font-size:14px;line-height:1.7;}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact;}}</style></head><body><div class="hero">${ph}<h1>${result.name}</h1><div class="prof">${result.profession}</div><p style="color:rgba(255,255,255,0.7);margin-bottom:12px">${result.tagline}</p>${contacts ? `<p style="color:rgba(255,255,255,0.5);font-size:13px">${contacts}</p>` : ""}</div><div class="body"><div class="sec">О себе</div><p class="bio">${result.bio}</p><div class="sec">Навыки</div><div class="skills">${(result.skills||[]).map(s=>`<span class="skill">${s}</span>`).join("")}</div><div class="sec">Почему я?</div>${(result.whyMe||[]).map(r=>`<div class="why-item">✓ ${r}</div>`).join("")}<div class="sec">Что предлагаю</div><div class="offer">${result.offer}</div></div></body></html>`);
    win.document.close(); setTimeout(() => win.print(), 600);
  };
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        ...
        <input style={S.input} placeholder="Город" value={form.city} onChange={e => set("city", e.target.value)} />
        <input style={S.input} placeholder="Telegram" value={form.telegram} onChange={e => set("telegram", e.target.value)} />
        <input style={S.input} placeholder="Instagram" value={form.instagram} onChange={e => set("instagram", e.target.value)} />
      </div>
      <textarea style={S.textarea} rows={3} placeholder="О себе" value={form.about} onChange={e => set("about", e.target.value)} />
      <input style={S.input} placeholder="Навыки" value={form.skills} onChange={e => set("skills", e.target.value)} />
      ...
      {result && !result.error && <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        <div style={{ ...S.card, textAlign: "center" }}>
          {photo && <img src={photo} style={{ width: 80, height: 80, borderRadius: "50%", objectFit: "cover", border: "3px solid #7C3AED", marginBottom: 12 }} />}
        ...
        <div style={S.card}><p style={{ color: "#7C3AED", fontSize: 11, textTransform: "uppercase", letterSpacing: 2, marginBottom: 10 }}>О себе</p><p style={{ color: "rgba(255,255,255,0.8)", fontSize: 13, lineHeight: 1.8 }}>{result.bio}</p></div>
        <div style={S.card}><p style={{ color: "#7C3AED", fontSize: 11, textTransform: "uppercase", letterSpacing: 2, marginBottom: 10 }}>Навыки</p><div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>{(result.skills||[]).map((s,i) => <span key={i} style={{ background: "rgba(124,58,237,0.2)", border: "1px solid rgba(124,58,237,0.4)", color: "#c4b5fd", padding: "6px 14px", borderRadius: 999, fontSize: 13 }}>{s}</span>)}</div></div>
        <div style={S.card}><p style={{ color: "#7C3AED", fontSize: 11, textTransform: "uppercase", letterSpacing: 2, marginBottom: 10 }}>Почему я?</p><div style={{ display: "flex", flexDirection: "column", gap: 8 }}>{(result.whyMe||[]).map((r,i) => <div key={i} style={{ background: "rgba(255,255,255,0.04)", borderLeft: "3px solid #7C3AED", padding: "10px 14px", borderRadius: "0 12px 12px 0", color: "rgba(255,255,255,0.8)", fontSize: 13 }}>✓ {r}</div>)}</div></div>
        <div style={{ ...S.card, background: "linear-gradient(135deg,rgba(124,58,237,0.2),rgba(147,51,234,0.2))" }}><p style={{ color: "#7C3AED", fontSize: 11, textTransform: "uppercase", letterSpacing: 2, marginBottom: 10 }}>Что предлагаю</p><p style={{ color: "rgba(255,255,255,0.85)", fontSize: 13, lineHeight: 1.7 }}>{result.offer}</p></div>
        <button onClick={print} style={{ width: "100%", padding: "14px 0", borderRadius: 18, background: "#fff", color: "#7C3AED", fontWeight: 700, fontSize: 14, border: "none", cursor: "pointer" }}>📥 Скачать PDF</button>
      </div>}
      {result?.error && <p style={{ color: "#f87171", fontSize: 13, textAlign: "center" }}>{result.error}</p>}
    </div>
  );
...
function TranslateTool() {
  ...
  const translate = async () => {
    ...
    const ln = { ru: "русский", uz: "узбекский латиница", en: "английский" };
    const res = await callAI(`Переведи с ${ln[from]} на ${ln[to]}. Только перевод:\n\n${text}`, "Translator. Return only translated text.");
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <select value={from} onChange={e => setFrom(e.target.value)} style={ss}>{LANGUAGES.map(l => <option key={l.code} value={l.code} style={{ background: "#1a1a2e" }}>{l.label}</option>)}</select>
        <button onClick={swap} style={{ width: 40, height: 40, borderRadius: "50%", background: "rgba(124,58,237,0.3)", color: "#fff", border: "none", cursor: "pointer", fontSize: 18, flexShrink: 0 }}>⇄</button>
        <select value={to} onChange={e => setTo(e.target.value)} style={ss}>{LANGUAGES.map(l => <option key={l.code} value={l.code} style={{ background: "#1a1a2e" }}>{l.label}</option>)}</select>
      </div>
...
function PoemTool() {
  ...
  const generate = async () => {
    ...
    setLoading(true); setResult("");
    const ln = { ru: "русском", uz: "узбекском латиница", en: "английском" };
    const res = await callAI(`Напиши стихотворение на ${ln[lang]} на тему: "${topic}". 3-4 строфы. Только стихотворение.`, "Poet. Beautiful poetry only.");
    setResult(res.trim()); setLoading(false);
  };
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
...
function NamesTool() {
  ...
  const generate = async () => {
    setLoading(true); setResult(null);
    const gl = gender === "boy" ? "мальчика" : "девочки";
    const raw = await callAI(`Предложи 5 имён для ${gl}, папа — ${dad}, мама — ${mom}. ТОЛЬКО JSON: [{"name":"","meaning":"","origin":""}] Язык: ${lang}. uz=латиница только. ru=кириллица только.`, `Names expert. JSON array only. ${lang}. uz=Latin. ru=Cyrillic.`);
    const parsed = parseJSON(raw);
    setResult(Array.isArray(parsed) ? parsed : [{ name: "Ошибка", meaning: "Попробуй снова", origin: "" }]);
    setLoading(false);
  };
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <input style={S.input} placeholder="Имя папы" value={dad} onChange={e => setDad(e.target.value)} />
      <input style={S.input} placeholder="Имя мамы" value={mom} onChange={e => setMom(e.target.value)} />
      <button onClick={generate} disabled={loading || !dad || !mom} style={loading || !dad || !mom ? S.btnPrimaryDisabled : S.btnPrimary}>{loading ? "⏳ Подбираю..." : "✨ Найти имена"}</button>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0f0c29 0%, #302b63 50%, #24243e 100%)", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", color: "#fff" }}>
      <div style={{ padding: "32px 20px 16px" }}>
        <h1 style={{ fontSize: 26, fontWeight: 900, margin: 0 }}>✦ Smart Tools</h1>
