...
function ResumeTool() {
  ...
  const generate = async () => {
    ...
    setResult(null);
    try {
      const prompt = `Создай профессиональное резюме на ${langLabels[form.lang]} языке.
Верни ТОЛЬКО JSON без markdown:
{"name":"","jobTitle":"","contact":"Телефон | Email | Город","summary":"","experience":[{"role":"","company":"","desc":""}],"education":"","skills":[]}
Данные: Имя: ${form.firstName} ${form.lastName}, Телефон: ${form.phone}, Email: ${form.email}, Город: ${form.city}, Должность: ${form.jobTitle}, Компания: ${form.company}, Опыт: ${form.experience}, Образование: ${form.education}, Навыки: ${form.skills}`;
...
<div class="page"><div class="sidebar"><div style="display:flex;flex-direction:column;align-items:center;text-align:center">${photoHTML}<h1>${result.name}</h1><div class="t">${result.jobTitle}</div></div><div class="sec">Контакты</div><p>${result.contact?.split("|").join("<br/>")}</p><div class="sec">Навыки</div><div>${(result.skills||[]).map(s=>`<span class="sk">${s}</span>`).join("")}</div></div>
<div class="main"><div class="sh">О себе</div><p class="sum">${result.summary}</p><div class="sh">Опыт</div>${(result.experience||[]).map(e=>`<div class="ei"><div class="er">${e.role}</div><div class="ec">${e.company}</div><div class="ed">${e.desc}</div></div>`).join("")}<div class="sh">Образование</div><p class="sum">${result.education}</p></div></div>
</body></html>`);
    win.document.close();
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ display: "flex", gap: 8 }}>
        {LANGUAGES.map((l) => (
          <button key={l.code} onClick={() => set("lang", l.code)} style={S.btnLang(form.lang === l.code)}>{l.label}</button>
        ))}
      </div>
      ...
      <button onClick={generate} disabled={loading || !form.firstName} style={loading || !form.firstName ? S.btnPrimaryDisabled : S.btnPrimary}>
      ...
      {result && !result.error && (
        <div style={S.card}>
          ...
          <button onClick={printResume} style={{ width: "100%", padding: "12px 0", borderRadius: 14, background: "#fff", color: "#7C3AED", fontWeight: 700, fontSize: 14, border: "none", cursor: "pointer" }}>
            📥 Скачать PDF
          </button>
        </div>
      )}
 ...
 ng: 2, marginBottom: 10 }}>О себе</p>
          ...
          <div style={S.card}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {(result.skills||[]).map((s,i) => (
                <span key={i} style={{ background: "rgba(124,58,237,0.2)", border: "1px solid rgba(124,58,237,0.4)", color: "#c4b5fd", padding: "6px 14px", borderRadius: 999, fontSize: 13 }}>{s}</span>
              ))}
            </div>
          </div>
          <div style={S.card}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {(result.whyMe||[]).map((r,i) => (
                <div key={i} style={{ background: "rgba(255,255,255,0.04)", borderLeft: "3px solid #7C3AED", padding: "10px 14px", borderRadius: "0 12px 12px 0", color: "rgba(255,255,255,0.8)", fontSize: 13 }}>✓ {r}</div>
              ))}
            </div>
          </div>
          ...
          <button onClick={printPortfolio} style={{ width: "100%", padding: "14px 0", borderRadius: 18, background: "#fff", color: "#7C3AED", fontWeight: 700, fontSize: 14, border: "none", cursor: "pointer" }}>
            📥 Скачать PDF
          </button>
        </div>
      )}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {result && (
        <div style={S.card}>
          ...
          <button onClick={() => navigator.clipboard.writeText(result)} style={{ marginTop: 10, color: "#a78bfa", fontSize: 12, background: "none", border: "none", cursor: "pointer" }}>📋 Копировать</button>
        </div>
      )}
    </div>
  );
...
function PoemTool() {
  ...
  const generate = async () => {
    ...
    const res = await callClaude(
      `Напиши красивое лирическое стихотворение на ${langNames[lang]} языке на тему: "${topic}". 3-4 строфы, красивые рифмы. Верни только стихотворение.`,
...
function NamesTool() {
  ...
  const generate = async () => {
    ...
    const res = await callClaude(
      `Предложи 5 красивых имён для ${genderLabel}, папа — ${dadName}, мама — ${momName}.
Верни ТОЛЬКО JSON без markdown:
[{"name":"Имя","meaning":"значение","origin":"происхождение"}]
ВАЖНО: язык ${lang}. Если uz — ТОЛЬКО латиница. Если ru — ТОЛЬКО кириллица. Если en — только английский. Никогда не смешивай алфавиты.`,
      `You are a names expert. Return only valid JSON array. Language: ${lang}. If uz - ONLY Latin. If ru - ONLY Cyrillic. Never mix scripts.`
    );
    try {
